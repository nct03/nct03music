...
import { nextSong, prevSong } from '../redux/playerSlice';
...
export default function MusicPlayer() {
  ...
  const { songs, currentSongIndex } = useSelector((state: RootState) => state.player)
  ...
  const loadSong = async (uri: string) => {
    const { sound } = await Audio.Sound.createAsync({ uri }, { shouldPlay: isPlaying });
  ...
  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
      ...
      <View style={{ flexDirection: "row", width: '100%', alignContent: "space-around", justifyContent: "space-around", marginTop: 30 }}>
        <FontAwesome6 name="shuffle" size={28} color={isShuffle ? "#fff" : "rgba(255,255,255,0.5)"} onPress={toggleShuffle} />
        <AntDesign name="stepbackward" size={28} color="#fff" onPress={handlePrevSong} />
        <AntDesign name={isPlaying ? 'pausecircle' : 'caretright'} size={28} color="#fff" onPress={playPause} />
        <AntDesign name="stepforward" size={28} color="#fff" title="Next" onPress={handleNextSong} />
